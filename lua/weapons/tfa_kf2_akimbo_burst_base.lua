...
-- Define the akimboammoside table with 6 entries
local akimboammoside = {1, 1, 1, 0, 0, 0}
...
function SWEP:ChooseShootAnim(ifp)
    ...
    if self2.GetStatL(self, "IsAkimbo") then
        ...
        if animData then
            ...
            if (self:GetAnimCycle() == 1 and clip <= 1) or (self:GetAnimCycle() == 0 and clip <= 4) then
...
function SWEP:ChooseReloadAnim()
    ...
    local animKey = (clip == 0 and "empty") or (clip <= 4 and "empty_half") or "half"
    ...
    local animCycleIndex = (totalAmmoAfterReload - 1) % #akimboammoside + 1

    -- Update AnimCycle based on the calculated index
    self:SetAnimCycle(akimboammoside[animCycleIndex])
...
function SWEP:ChooseInspectAnim()
	...
	elseif self:GetActivityEnabled(ACT_VM_FIDGET_EMPTY) and self:Clip1() <= 3 then 
...
function SWEP:Think2(...)
    if IsValid(self.OwnerViewModel) then
        ...
        local pmEmptyRightValue = (clip <= 3) and 1 or 0
        local pmEmptyLeftValue = (clip == 0) and 1 or 0
...
function SWEP:ToggleAkimbo(arg1) -- triggered every shot
    if self:GetBurstCount() == self:GetMaxBurst() - 1 then --2 is actually the third shot here for some reason
        -- Set a flag to indicate the burst is complete
...
function SWEP:Deploy()
    ...
    local animCycleIndex = (clip - 1) % #akimboammoside + 1
...
function SWEP:PostPrimaryAttack(...)
	...
	local burst_to_fix = self:GetAmmoCycles(true)
	self:SetBurstCount(burst_to_fix)

    -- Only update AnimCycle if the burst is complete
    if self:GetBurstCount() == self:GetMaxBurst() then 
        -- Calculate the index for the akimboammoside table using modulo
        local animCycleIndex = (clip - 1) % #akimboammoside + 1
...
-- Because editing the HUD Ammo code requires copying the entire function over. WHY
...
function SWEP:GetAmmoCycles(flag)
	local total_ammo = self:Clip1()
	local full_cycles = math.floor(total_ammo / 6)
	local remaining_shots = total_ammo % 6

	-- Ammo in each gun after full cycles
	local ammo_left_full_cycle = full_cycles * 3
	local ammo_right_full_cycle = full_cycles * 3

	-- Distribute the remaining shots
	local ammo_left_additional = 0
	local ammo_right_additional = 0

	if remaining_shots <= 3 then
		-- All remaining shots go to the left gun
		ammo_left_additional = remaining_shots
	else
		-- First 3 shots go to the left gun, rest to the right
		ammo_left_additional = 3
		ammo_right_additional = remaining_shots - 3
	end

	-- Total ammo in each gun
	local ammo_left = ammo_left_full_cycle + ammo_left_additional
	local ammo_right = ammo_right_full_cycle + ammo_right_additional

	-- Watch for partial reloads that break the burst count logic
	if not (ammo_left_additional == self:GetMaxBurst()) and not (ammo_left_additional == 0) then
		burst_to_fix = ammo_left_additional
	elseif not (ammo_right_additional == self:GetMaxBurst()) and not (ammo_right_additional == 0) then
		burst_to_fix = ammo_right_additional
	else
		burst_to_fix = self:GetMaxBurst()
	end

	if flag then
		return burst_to_fix
	else
		return ammo_left, ammo_right
	end
end
...
function SWEP:DrawHUDAmmo()
	...
	local ammocycles_left, ammocycles_right = self:GetAmmoCycles(false)
	...
	if self:GetInspectingProgress() < 0.01 and self2.GetStatL(self, "Primary.Ammo") ~= "" and self2.GetStatL(self, "Primary.Ammo") ~= 0 then
		...
		if self2.GetStatL(self, "Primary.ClipSize") and self2.GetStatL(self, "Primary.ClipSize") ~= -1 then
			...
			if self2.GetStatL(self, "IsAkimbo") and self2.GetStatL(self, "EnableAkimboHUD") ~= false then
				str = clipstr:format(math.ceil(ammocycles_right))
		...
		if self2.GetStatL(self, "IsAkimbo") and self2.GetStatL(self, "EnableAkimboHUD") ~= false then
			...
			if self2.GetStatL(self, "Primary.ClipSize") and self2.GetStatL(self, "Primary.ClipSize") ~= -1 then
				...
				str = clipstr:format(math.floor(ammocycles_left))
