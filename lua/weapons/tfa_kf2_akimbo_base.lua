...
function SWEP:ChooseShootAnim(ifp)
    ...
    if self2.GetStatL(self, "IsAkimbo") then
        ...
        if animData then
            ...
            if (self:GetAnimCycle() == 1 and clip <= 1) or (self:GetAnimCycle() == 0 and clip <= 2) then
...
function SWEP:ChooseReloadAnim()
    ...
    local animKey = (clip == 0 and "empty") or (clip == 1 and "empty_half") or "half"
    ...
    -- Set AnimCycle based on whether the total ammo after reload is even or odd
    if totalAmmoAfterReload % 2 == 0 then
        self:SetAnimCycle(0)  -- Set to 1 for even number of rounds
    else
        self:SetAnimCycle(1)  -- Set to 0 for odd number of rounds
    end
...
function SWEP:ChooseInspectAnim()
	...
	elseif self:GetActivityEnabled(ACT_VM_FIDGET_EMPTY) and self:Clip1() <= 1 then 
...
function SWEP:Think2(...)
    if IsValid(self.OwnerViewModel) then
        ...
        local pmEmptyRightValue = (clip <= 1) and 1 or 0
...
function SWEP:PostPrimaryAttack(...)
    ...
    timer.Simple(.1, function()
        if IsValid(self) then  -- Check if the weapon is still valid
            local newAnimCycle = (clip % 2 == 0) and 0 or 1
            self:SetAnimCycle(newAnimCycle)
        end
    end)
